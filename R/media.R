...
.clh_manifest_prompt <- function(entry, default_prompt) {
...
cl_chat_transcribe_audio <- function(chat,
  ...
  .clh_manifest_save(manifest, manifest_path)
...
cl_chat_describe_images <- function(chat,
  ...
  cache_base <- .clh_resolve_store_dir(chat, cache_dir)
  attr(chat, "chat_key") <- chat_key
  desc_dir <- .clh_ensure_dir(file.path(cache_base, "image_descriptions"))
  ...
  image_rows <- .clh_unique_attachment_rows(att, image_rows, zip_id = zip_id)
  ...
  for (idx in seq_along(image_rows)) {
    ...
    att_path <- att$attachment_path[r]
    att_key <- .clh_attachment_id(att_name, att_path, att$attachment_key[r], zip_id = zip_id)
    progress <- .clh_progress_with_eta(idx, length(image_rows), image_durations)
    ...
    if (!overwrite && !is.null(existing) &&
      ...
      identical(existing$service, service) &&
      identical(existing$model, model) &&
      identical(.clh_manifest_prompt(existing, .clh_default_image_prompt()), prompt) &&
      !is.null(existing$description) && nzchar(existing$description)) {
      description_map[[att_key]] <- existing$description
      if (verbose) message("Skipping (already processed) ", progress, ": ", att_name)
      run_items[[att_key]] <- c(existing, list(prompt = .clh_manifest_prompt(existing, .clh_default_image_prompt()), reused = TRUE))
      next
    }
    ...
    if (!overwrite && file.exists(desc_path)) {
      ...
      if (!is.null(existing)) {
        prompt_matches_cache <- identical(.clh_manifest_prompt(existing, .clh_default_image_prompt()), prompt)
      } else if (identical(prompt, .clh_default_image_prompt())) {
    ...
    raw <- tryCatch(
      ...
      error = function(e) {
        error_message <<- conditionMessage(e)
        warning("Image description failed for ", basename(att_path), ": ", error_message, call. = FALSE)
        NA_character_
      }
    )
    elapsed <- as.numeric(difftime(Sys.time(), started_at, units = "secs"))
    ...
    if (!.clh_is_error_response(raw, description)) {
      ...
      if (is.na(error_message) || !nzchar(error_message)) {
        error_message <- .clh_error_response_message(raw, description)
      }
      if (verbose) {
        message(
          "Image description error recorded ",
          progress,
          ": ",
          att_name,
          " [",
    ...
    if (verbose) {
      eta_left <- .clh_estimated_remaining(image_durations, length(image_rows) - idx)
